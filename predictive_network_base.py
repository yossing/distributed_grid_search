...
from visualisation import plot_loss #import visualisation as vis
# reload(vis)
import optimizer_lasagne_reformatted as optimizer_lasagne
...
#-------------------------------------------------------------------------------
    ...
    def train_network(self, X_train, y_train, X_val= None, y_val = None, 
        ...
        if num_epochs>0:
            ...
            # for key, value in cost_history.items():
            #     self.cost_history[key].append(value)
            if show_graph:
    ...
    def save_network_results_and_settings(self, save_path=None):
        '''Pickle the network parameters and settings'''
        ...
        pkl.dump(self.get_network_results_and_settings(), open(save_path, 'wb'))
        return
...
def load_network_results_and_settings(save_path):
